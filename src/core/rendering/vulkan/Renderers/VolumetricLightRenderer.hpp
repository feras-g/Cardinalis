...
struct VolumetricLightRenderer : IRenderer
{
	...
	void create_pipeline_volumetric_point_lights()
	{
		...
		for (int i = 0; i < NUM_FRAMES; i++)
		{
			...
			volumetric_point_light_descriptor_set[i].write_descriptor_combined_image_sampler(1, DeferredRenderer::gbuffer[i].depth_attachment.view, VulkanRendererCommon::get_instance().s_SamplerClampNearest);
		}
	...
	virtual void create_renderpass()
	{
		...
		for (int i = 0; i < NUM_FRAMES; i++)
		{
			...
			volumetric_lighting_attachment[i].create(ctx.device, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT);
			ui_texture_ids[i] = ImGui_ImplVulkan_AddTexture(VulkanRendererCommon::get_instance().s_SamplerClampNearest, volumetric_lighting_attachment[i].view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
