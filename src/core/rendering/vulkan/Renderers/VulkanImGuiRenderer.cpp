...
void VulkanImGuiRenderer::create_scene_viewport_attachment()
{
	for (int i = 0; i < NUM_FRAMES; i++)
	{
		...
		scene_viewport_attachments_ids[i] = ImGui_ImplVulkan_AddTexture(VulkanRendererCommon::get_instance().smp_clamp_linear, scene_viewport_attachments[i].view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
	}
}
