...
namespace vk
{
	void renderpass_dynamic::begin(VkCommandBuffer cmd_buffer, glm::vec2 extent, uint32_t viewMask)
	{
		VkRect2D area = {};
		area.offset.x = 0;
		area.offset.y = 0;
	...
	void renderpass_dynamic::end(VkCommandBuffer cmd_buffer) const
	{
		vkCmdEndRendering(cmd_buffer);
	}

	void renderpass_dynamic::reset()
	{
	...
	void renderpass_dynamic::add_attachment(VkImageView view, VkImageLayout layout, VkAttachmentLoadOp loadOp)
	{
		assert(view);
	...
	void renderpass_dynamic::add_color_attachment(VkImageView view, VkAttachmentLoadOp loadOp)
	{
		add_attachment(view, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, loadOp);
	}

	void renderpass_dynamic::add_depth_attachment(VkImageView view, VkAttachmentLoadOp loadOp)
	{
		add_attachment(view, VK_IMAGE_LAYOUT_DEPTH_ATTACHMENT_OPTIMAL, loadOp);
	}

	void renderpass_dynamic::add_stencil_attachment(VkImageView view, VkAttachmentLoadOp loadOp)
	{
