...
namespace vk
{
	swapchain::swapchain(VkSurfaceKHR surface)
		: h_surface(surface)
	{
		const VkInstance& instance = ctx.device.instance;
		...
		VK_CHECK(fpGetPhysicalDeviceSurfacePresentModesKHR(ctx.device.physical_device, h_surface, &presentModeCount, nullptr));
		assert(presentModeCount >= 1);

		std::vector<VkPresentModeKHR> presentModes(presentModeCount);
		VK_CHECK(fpGetPhysicalDeviceSurfacePresentModesKHR(ctx.device.physical_device, h_surface, &presentModeCount, presentModes.data()));
		info.present_mode = VK_PRESENT_MODE_FIFO_KHR;
		LOG_WARN("Current present mode : {0}", vk_object_to_string(info.present_mode));
	}

	void swapchain::init(VkFormat colorFormat, VkColorSpaceKHR colorSpace, VkFormat depthStencilFormat)
	{
		// Query/Set surface capabilities
		VkSurfaceCapabilitiesKHR caps;
		VK_CHECK(fpGetPhysicalDeviceSurfaceCapabilitiesKHR(ctx.device.physical_device, h_surface, &caps));
		...
		fpGetPhysicalDeviceSurfaceFormatsKHR(ctx.device.physical_device, h_surface, &surfaceFormatsCount, nullptr);
		std::vector<VkSurfaceFormatKHR> surfaceFormats(surfaceFormatsCount);
		fpGetPhysicalDeviceSurfaceFormatsKHR(ctx.device.physical_device, h_surface, &surfaceFormatsCount, surfaceFormats.data());
		...
		VK_CHECK(vkCreateSwapchainKHR(ctx.device, &swapchainCreateInfo, nullptr, &vk_swapchain));
		...
		VK_CHECK(fpGetSwapchainImagesKHR(ctx.device, vk_swapchain, &info.image_count, tmp.data()));
		...
		for (uint32_t i = 0; i < info.image_count; i++)
		{
			...
			color_attachments[i].create_view(ctx.device, { VK_IMAGE_VIEW_TYPE_2D, VK_IMAGE_ASPECT_COLOR_BIT });
			...
			depth_attachments[i].create(ctx.device, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT);
			depth_attachments[i].create_view(ctx.device, { VK_IMAGE_VIEW_TYPE_2D, VK_IMAGE_ASPECT_DEPTH_BIT });
		}
	...
	void swapchain::clear_color(VkCommandBuffer cmd_buffer)
	{
		vk::renderpass_dynamic rp;
		rp.add_color_attachment(color_attachments[ctx.curr_frame_idx].view);
	...
	void swapchain::destroy()
	{
		...
		vkDestroySwapchainKHR(ctx.device, vk_swapchain, nullptr);
	}

	VkResult swapchain::acquire_next_image(VkSemaphore imageAcquiredSmp)
	{
		return vkAcquireNextImageKHR(ctx.device, vk_swapchain, UINT64_MAX, imageAcquiredSmp, VK_NULL_HANDLE, &current_backbuffer_idx);
	}

	VkResult swapchain::present(VkCommandBuffer cmdBuffer, VkQueue queue, uint32_t imageIndices)
	{
